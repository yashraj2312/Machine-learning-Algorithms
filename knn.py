...
trainDataset, trainTarget = np.array(dataset[:trainLen, :-1], dtype='float'), dataset[:trainLen, -1]
testDataset, testTarget = np.array(dataset[trainLen:, :-1], dtype='float'), dataset[trainLen:, -1]
...
for row, target in zip(testDataset, testTarget):
    ...
    #print("test")
    print("Actual:",target.ljust(15)," ","Predicted:",predicted)
